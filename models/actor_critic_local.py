...
class actor_critic(nn.Module):
    def __init__(self, args):
        ...
        self.noop_emb = nn.Parameter(torch.zeros(feat_dim))
        nn.init.normal_(self.noop_emb, std=1e-3)
        self.state_norm = nn.LayerNorm(args.out_dim, eps=1e-3)
        nn.init.ones_(self.state_norm.weight)
        nn.init.zeros_(self.state_norm.bias)
        self.actor = nn.Sequential(
            nn.Linear(args.out_dim, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 1)
    ...
    def act(self, current_state, origin_p, chosen_query=None,deterministic=False):
        ...
        probs = origin_action.squeeze(0)  # [N]
    ...
    def evaluate_action(self, states, actions, origin_p):
        for i in range(len(states)):
            subquery_features, query_mask,chosen_query = states[i]
            action = actions[i]
